...
const HomePage = memo(({ onLogin, onRegister }) => {
  ...
  return (
    <div className="home-page">
      ...
      {/* NAV — theme toggle only; Sign In & Get Started removed */}
              ...
              <div className="mock-items">
                {[
                  ...
                  <div key={item.name} className={`mock-item mock-item--${item.status}`}>
                    ...
                    <span className={`badge badge-${item.status==='safe'?'safe':'warn'}`}>{item.days}d</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      {/* FEATURES — 5 cards, centered layout */}
      <section className="features-section">
        <div className="section-inner">
          ...
          <div className="features-grid features-grid--5">
            {FEATURES.map(f => (
          ...
          <div className="steps-grid">
            {STEPS.map((s,i) => (
              <div key={s.n} className="step-card">
                ...
                <div className="step-icon-wrap"><i className={`fas ${s.icon}`} /></div>
                <h3>{s.title}</h3>
                <p>{s.desc}</p>
              </div>
            ))}
            ...
            <ul className="alert-list">
              {[
                ...
                <li key={a.text} className={`alert-item alert-item--${a.color}`}>
                  <i className={`fas ${a.icon}`} /><span>{a.text}</span>
                </li>
              ))}
            </ul>
          </div>
            ...
            <div className="notif-mock card">
              ...
              {[
                ...
                <div key={n.text} className="notif-item">
                  ...
                  <div><strong>{n.text}</strong><small>{n.sub}</small></div>
                </div>
              ))}
            </div>
          </div>
